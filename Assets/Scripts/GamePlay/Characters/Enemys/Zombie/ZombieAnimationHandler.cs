...
namespace GamePlay.Characters.Enemys
{
    public class ZombieAnimationHandler : AnimationHandler
    {
        ...
        private float _speedValue => _animator.GetFloat(_speedKey);
        ...
        public void PlayMove()
        {
            ...
            DOVirtual.Float(_speedValue, 0, 1, (amount) => { _animator.SetFloat(_speedKey, amount); });
        }

        public void PlayRun()
        {
            Play(ZombieStateType.Run);
            DOVirtual.Float(_speedValue, 1, 1, (amount) => { _animator.SetFloat(_speedKey, amount); });
        }
    }
}