...
namespace UnityTimer
{
    public class Timer
    {
        ...
        )
        {
            ...
            Timer timer = new Timer(duration, onComplete, onUpdate, isLooped, useRealTime,isFloored, autoDestroyOwner);
        ...
        private void Update()
        {
            ...
            if (this._onUpdate != null)
            {
                ...
                if (isFloored && Time.time >= _nextUpdateTime)
                {
                    Debug.Log(Time.time + ">=" + _nextUpdateTime);
                    _nextUpdateTime = Mathf.FloorToInt(Time.time) + 1;
